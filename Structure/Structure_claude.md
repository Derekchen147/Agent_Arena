...
以「人类 @架构师 请帮我拆解这个需求」为例：

```
1. 人类在前端输入消息，点击发送
   │
2. 前端 → API Gateway（WebSocket）
   │  消息内容: "请帮我拆解这个需求：做一个用户管理系统"
   │  mentions: ["architect"]
   │
3. API Gateway → SessionManager
   │  存储消息到数据库
   │  返回 message_id
   │
4. API Gateway → Orchestrator.on_new_message()
   │
5. Orchestrator 解析 mentions
   │  → architect 在 mentions 中 → must_reply
   │  → developer、tester 不在 → may_reply
   │
6. Orchestrator 创建 Turn
   │  must_reply: [architect]
   │  may_reply:  [developer, tester]
   │
7. 对每个要调用的 Agent：
   │
   ├─ ContextBuilder.build_input("architect", ...)
   │  │  截断历史消息，检索相关记忆
   │  │  组装 AgentInput（invocation="must_reply"）
   │  │
   │  └─ WorkerRuntime.invoke_agent("architect", input)
   │     │  启动/复用 Adapter 进程
   │     │  emit_status("architect", "analyzing")
   │     │  调用 Anthropic API
   │     │  emit_status("architect", "generating")
   │     │  收到回复
   │     │  emit_status("architect", "done")
   │     │
   │     └─ 返回 AgentOutput:
   │        content: "好的，我来拆解这个需求...（结构化输出）"
   │        next_mentions: ["developer"]    ← 架构师 @ 了开发
   │
   ├─ （并行）对 may_reply 的 Agent 做轻量评估
   │  developer: 关键词匹配 "用户管理系统" → 相关性 0.7 > 阈值 → should_respond
   │  tester: 关键词匹配 → 相关性 0.3 < 阈值 → 不回复
   │
8. Orchestrator 收到 architect 的回复
   │  → 消息写入 SessionManager
   │  → 推送给前端（WebSocket）
   │  → architect 的 next_mentions 包含 "developer"
   │  → 创建新的 Turn，developer 为 must_reply
   │
9. 新 Turn 执行：developer 收到架构师的输出，开始响应
   │  ...（流程同上）
   │
10. developer 回复后，如果没有 next_mentions 且无人 may_reply
    → 本回合结束，等待人类下一条消息
```
